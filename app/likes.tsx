...
export default function Likes({ tweet, addOptimisticTweet }: { tweet: TweetWithAuthor; addOptimisticTweet: (newTweet: TweetWithAuthor) => void; }) {
    ...
    return (
        ...
        <svg 
          ...
          className={`group-hover:fill-red-600 group-hover:stroke-red-600 ${tweet.user_has_liked_tweet ? "fill-red-600 stroke-red-600" : 
        ...
        <span className={`ml-2 text-sm group-hover:text-red-600 ${tweet.user_has_liked_tweet ? 'text-red-600' : 'text-gray-500'}`}>{tweet.likes}
        </span>
        </button>
    );
}